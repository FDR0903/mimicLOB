...
class OrderTree(object):
    ...
    # True if the ordre should loose priority
    def update_order_looses_priority(self, order_update):
        order = self.order_map[order_update['order_id']]
        original_quantity = order.quantity
        return ((order_update['price'] != order.price) | (order_update['quantity'] > original_quantity))
            
    def update_order_quantity(self, order_update):
        order = self.order_map[order_update['order_id']]
        original_quantity = order.quantity
    ...
    def update_order(self, order_update):
        ...
        if ((order_update['price'] != order.price) | (order_update['quantity'] > original_quantity)):
            ...
            # FDR : insert order will first delete order in the order map
            #       and the order_list 
            self.insert_order(order_update)
        else:
            # Quantity reduced. Price is the same.
            order.update_quantity(order_update['quantity'], order_update['timestamp'])
        self.volume += order.quantity - original_quantity

    # return the trader id of the order, to notify him
